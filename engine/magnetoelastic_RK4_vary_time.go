...
func (_ *magelasRK4_vary_time) Step() {
	...
	if FixDtU != 0 && FixDtM != 0 && FixDtU > FixDtM || FixDtU == 0 || FixDtM == 0 {
		...
		for t0M+float64(FixDtM) > t0U && undoMagnetic == false {
			dt = float32(Dt_si)
			//h := float32(Dt_si * GammaLL)
			...
			if (err < MaxErr && err2 < MaxErr) || Dt_si <= MinDt || FixDt != 0 { // mindt check to avoid infinite loop
				...
				if !overwriteUndoBackup {
					if printedUndoMistakes {
						fmt.Printf("\rUndone: %v", undoMistakes)
					} else {
						fmt.Printf("Undone: %v", undoMistakes)
						printedUndoMistakes = true
					}
				}
			}
			t0U = Time
		}
		if Timetravel && !undoMagnetic {
			data.Copy(m, mFuture)
		}
	}
...
func (_ *magelasRK4_vary_time) StepRegion(region *SolverRegion) {
