...
func planValue(stream *tokenStream) (*evaluationStage, error) {
	...
	case GPUCOORDS:
		operator = makeParameterStageGPU(token.Value.(string))
	case NUMERIC:
		fallthrough
	case STRING:
		fallthrough
...
func findTypeChecks(symbol OperatorSymbol) typeChecks {
	...
	case EXPONENT:
		return typeChecks{
			left:  func(value interface{}) bool { return isFloat64(value) || isDataSlice(value) },
			right: func(value interface{}) bool { return isFloat64(value) || isDataSlice(value) },
		}
	case NEGATE:
		return typeChecks{
			right: isFloat64,
