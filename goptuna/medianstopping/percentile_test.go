...
import (
	...
	"github.com/mumax/3/goptuna/medianstopping"
)

func TestPercentilePruner_PruneWithOneTrial(t *testing.T) {
	study, err := goptuna.CreateStudy("", os.Stdout)
	if err != nil {
...
func TestPercentilePruner_Prune(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			study, err := goptuna.CreateStudy("", os.Stdout, goptuna.StudyOptionDirection(tt.direction))
			if err != nil {
				t.Errorf("should be err=nil, but got %s", err)
				return
			}
