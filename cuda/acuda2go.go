...
func cuda2go(fname string) {
	...
	for i := range args {
		...
		var importCTmp bool
		argt[i], importCTmp = typemap(args[i][0])
		if importCTmp {
			importC = true
		}
		argn[i] = args[i][1]
	}
	wrapgen(fname, funcname, argt, argn, importC)
}

// translate C type to Go type.
func typemap(ctype string) (string, bool) {
...
var tm = map[string]string{"float*": "unsafe.Pointer", "float": "float32", "int": "int", "int*": "unsafe.Pointer", "uint8_t*": "unsafe.Pointer", "uint8_t": "byte", "size_t": "C.size_t", "size_t*": "unsafe.Pointer"}
...
func wrapgen(filename, funcname string, argt, argn []string, importC bool) {
	...
	basename := util.NoExt(util.NoExt(filename))
	for _, f := range ls {
		match, e := regexp.MatchString(fmt.Sprintf(`^%s_\d{2}\.ptx\.a$`, regexp.QuoteMeta(basename)), f)
		util.PanicErr(e)
		if match {
			cc, ei := strconv.Atoi(f[len(f)-len("00.ptx.a") : len(f)-len(".ptx.a")])
			util.PanicErr(ei)
...
func k_{{.Name}}_async ( {{range $i, $t := .ArgT}}{{index $.ArgN $i}} {{$t}}, {{end}}key string, cfg *config) {
	...
	cu.LaunchKernel({{.Name}}_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, Get_Stream(key), args)
