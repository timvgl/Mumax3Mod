...
func FFT_T_Step_MEM_Real(dst, src2 *data.Slice, minF, dF, fftT float32, key string) {
	size := dst.Size()
	size[X] /= 2
	util.Assert(src2.NComp() == dst.NComp())
	util.Assert(src2.Size() == dst.Size())
	...
	size[X] *= 2
	for c := range dst.NComp() {
		k_FFT_Step_MEM_Real_async(dst.DevPtr(c), src2.DevPtr(c), size[X], size[Y], size[Z], dst.LengthF, minF, dF, fftT, key, cfg)
	}
	//SyncFFT_T(key)
}
